...
if ($method == 'add_clozapine') {

        $vn = $_POST['vn'];
        $hn = $_POST['hn'];
        $WBC = isset($_POST['WBC'])?$conv->utf8_to_tis620($_POST['WBC']):'';
        $ANC = isset($_POST['ANC'])?$conv->utf8_to_tis620($_POST['ANC']):'';
        $platelet = isset($_POST['platelet'])?$conv->utf8_to_tis620($_POST['platelet']):'';
        $WBC_val = isset($_POST['WBC_val'])?$_POST['WBC_val']:'';
        $ANC_val = isset($_POST['ANC_val'])?$_POST['ANC_val']:'';
        $platelet_val = isset($_POST['platelet_val'])?$_POST['platelet_val']:'';
        $last_CBC = $conv->utf8_to_tis620($_POST['last_CBC']);
        $last_CBC_val = isset($_POST['last_CBC_val'])?$_POST['last_CBC_val']:'';
        $mg_day = $conv->utf8_to_tis620($_POST['mg_day']);
        $weigh = $_POST['bw'];
        $height = $_POST['height'];
        $BMI = $_POST['bmi'];
        ...
        $data = array($hn,$vn,$WBC,$WBC_val,$ANC,$ANC_val,$platelet,$platelet_val,$last_CBC,$last_CBC_val,$mg_day,$weigh
        ,$height,$BMI,$symplomo01,$symplomo02,$symplomo03,$symplomo04,$symplomo05,$symplomo06,$regdate,$recorder);
        //$field = array('id','hcode','vdate','vn','hn','sex','dob','pdx','dx0','dx1','dx2','dx3','cgis_score','clinic','user','dupdate');
        $table = "jvl_clozapine";
        $clozapine = $connDB->insert($table, $data);
        $res = array("messege"=>$clozapine);
    if($clozapine===false){
        $res = array("messege"=>'ไม่สามารถบันทึกประเมินยา Clozapine ได้!!!!');
    }else{
        $res = array("messege"=>'บันทึกประเมินยา Clozapine สำเร็จ!!!!',"id"=>$clozapine);
    }
