...
if ($method == 'add_lithium') {

        $vn = $_POST['vn'];
        $hn = $_POST['hn'];
        $last_lithium = $conv->utf8_to_tis620($_POST['last_Lithium']);
        $last_lithium_val = isset($_POST['last_Lithium_val'])?$_POST['last_Lithium_val']:'';
        $lithium = isset($_POST['Lithium'])?$conv->utf8_to_tis620($_POST['Lithium']):'';
        $Lithium_val = isset($_POST['Lithium_val'])?$_POST['Lithium_val']:'';
        $last_BUN = $conv->utf8_to_tis620($_POST['last_BUN']);
        $last_BUN_val = isset($_POST['last_BUN_val'])?$_POST['last_BUN_val']:'';
        $BUN = isset($_POST['BUN'])?$conv->utf8_to_tis620($_POST['BUN']):'';
        $BUN_val = isset($_POST['BUN_val'])?$_POST['BUN_val']:'';
        $last_Cr = $conv->utf8_to_tis620($_POST['last_Cr']);
        $last_Cr_val = isset($_POST['last_Cr_val'])?$_POST['last_Cr_val']:'';
        $Cr = isset($_POST['Cr'])?$conv->utf8_to_tis620($_POST['Cr']):'';
        $Cr_val = isset($_POST['Cr_val'])?$_POST['Cr_val']:'';
        $eGFR = isset($_POST['eGFR'])?$conv->utf8_to_tis620($_POST['eGFR']):'';
        $eGFR_val = isset($_POST['eGFR_val'])?$_POST['eGFR_val']:'';
        $last_TF = $conv->utf8_to_tis620($_POST['last_Thyroid']);
        $last_TF_val = isset($_POST['last_Thyroid_val'])?$_POST['last_Thyroid_val']:'';
        $TSH = isset($_POST['TSH'])?$conv->utf8_to_tis620($_POST['TSH']):'';
        $TSH_val = isset($_POST['TSH_val'])?$_POST['TSH_val']:'';
        $FT3 = isset($_POST['FT3'])?$conv->utf8_to_tis620($_POST['FT3']):'';
        $FT3_val = isset($_POST['FT3_val'])?$_POST['FT3_val']:'';
        $FT4 = isset($_POST['FT4'])?$conv->utf8_to_tis620($_POST['FT4']):'';
        $FT4_val = isset($_POST['FT4_val'])?$_POST['FT4_val']:'';
        ...
        $symplomo09 = $conv->utf8_to_tis620($_POST['symplomo09']);
        $symplomo10 = $conv->utf8_to_tis620($_POST['symplomo10']);
        $recorder = $conv->utf8_to_tis620($_POST['user']);
        $regdate = date('Y-m-d H:i:s');

        $data = array($hn,$vn,$last_lithium,$last_lithium_val,$lithium,$Lithium_val,$last_BUN,$last_BUN_val,$BUN,$BUN_val,$last_Cr,$last_Cr_val
        ,$Cr,$Cr_val,$eGFR,$eGFR_val,$last_TF,$last_TF_val,$TSH,$TSH_val,$FT3,$FT3_val,$FT4,$FT4_val,$symplomo01,$symplomo02,$symplomo03,$symplomo04
        ,$symplomo05,$symplomo06,$symplomo07,$symplomo08,$symplomo09,$symplomo10,$regdate,$recorder);
    ...
    if($lithium_T===false){
        $res = array("messege"=>'ไม่สามารถบันทึกประเมินยา Lithium Carbonate ได้!!!!');
    }else{
        $res = array("messege"=>'บันทึกประเมินยา Lithium Carbonate สำเร็จ!!!!',"id"=>$lithium_T);
    }
