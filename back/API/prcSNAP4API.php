...
if ($method == 'add_snap4') {
    ...
    $hn = $_POST['hn'];
    $month = $_POST['month'];
    $assdate = insert_date($_POST['assdate']);
    $class_room = $_POST['class_room'];
    $assessor = $conv->utf8_to_tis620($_POST['assessor']);
    $ADHD_In = $_POST['ADHD_In'];
    $ADHD_HIn = $_POST['ADHD_HIn'];
    $ODD = $_POST['ODD'];
    ...
    $data = array($vdate,$vn,$hn,$month,$assdate,$class_room,$assessor,$ADHD_In,$ADHD_HIn,$ODD,$user,$dupdate);
    //$field = array('id','hcode','vdate','vn','hn','sex','dob','pdx','dx0','dx1','dx2','dx3','cgis_score','clinic','user','dupdate');
    $table = "jvl_snap_iv";
    $snap4 = $connDB->insert($table, $data);
    $res = array("messege"=>$snap4);
if($snap4===false){
    $res = array("messege"=>'ไม่สามารถบันทึก SNPA-IV ได้!!!!');
}else{
    $res = array("messege"=>'บันทึก SNPA-IV สำเร็จ!!!!',"id"=>$snap4);
}
